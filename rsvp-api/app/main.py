...
@app.get("/rsvp/{phone}", response_model=schemas.GuestResponse)
...
@app.post("/rsvp/confirm", response_model=schemas.GuestResponse)
...
def add_guest(payload: schemas.CreateGuestRequest):
    ...
    if existing:
        raise HTTPException(status_code=409, detail="Guest already exists")

    return crud.create_guest(
        phone=payload.phone,
        name=payload.name,
        invited_total=payload.invited_total,
    )
